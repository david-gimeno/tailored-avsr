...
from src.models.avsr_maskctc_model import AVSRMaskCTCInference
...
class Speech2Text:
    ...
    def __init__(
        ...
        # 1. Build ASR model
        asr_model, asr_train_args = AVSRTask.build_model_from_file(
            asr_train_config, asr_model_file, device
        )
        asr_model.to(dtype=getattr(torch, dtype)).eval()
        token_list = asr_model.token_list

        s2t = AVSRMaskCTCInference(
            asr_model=asr_model,
    ...
    def __call__(
        ...
        Args:
            audio: Input acoustic speech data
            video: Input visual speech data
        Returns:
        ...
        # Input acoustic speech data
        if isinstance(audio, np.ndarray):
            audio = torch.tensor(audio)

        # data: (Nsamples,) -> (1, Nsamples)
        audio = audio.unsqueeze(0).to(getattr(torch, self.dtype))
        # lenghts: (1,)
        audio_lengths = audio.new_full([1], dtype=torch.long, fill_value=audio.size(1))

        # Input visual speech data
        ...
        # lenghts: (1,)
        video_lengths = video.new_full([1], dtype=torch.long, fill_value=video.size(1))

        batch = {"audio": audio, "audio_lengths": audio_lengths, "video": video, "video_lengths": video_lengths}
...
def inference(
    ...
    loader = AVSRTask.build_streaming_iterator(
        data_path_and_name_and_type,
        dtype=dtype,
        batch_size=batch_size,
        key_file=key_file,
        num_workers=num_workers,
        preprocess_fn=AVSRTask.build_preprocess_fn(speech2text.asr_train_args, False),
        collate_fn=AVSRTask.build_collate_fn(speech2text.asr_train_args, False),
