...
class BaseTransformerDecoder(AbsDecoder, BatchScorerInterface):
    ...
    def forward_one_step(
        ...
        new_cache = []
        for j, (c, decoder) in enumerate(zip(cache, self.decoders)):
            if j == 0:
                x, tgt_mask, memory, memory_mask, s1 = decoder(
                    x, tgt_mask, memory, None, None, cache=c
                )
            else:
                x, tgt_mask, memory, memory_mask, s1 = decoder(
                    x, tgt_mask, memory, None, s1, cache=c
                )
...
class TransformerDecoderSimT(BaseTransformerDecoder):
    def __init__(
        self,
        vocab_size: int,
        encoder_output_size: int,
