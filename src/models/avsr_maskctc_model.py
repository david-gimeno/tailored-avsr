...
class AVSRMaskCTCModel(ESPnetAVSRModel):
    """Hybrid CTC/Masked LM Encoder-Decoder model (Mask-CTC) with a encoder that receives two data streams inputs."""
...
class AVSRMaskCTCInference(torch.nn.Module):
    """Mask-CTC-based non-autoregressive inference"""

    def __init__(
        self,
        asr_model: AVSRMaskCTCModel,
        n_iterations: int,
    ...
    def forward(self, enc_out: torch.Tensor) -> List[Hypothesis]:
        ...
        for i, y in enumerate(y_hat.tolist()):
            ...
            while cnt < ctc_ids.shape[1] and y == ctc_ids[0][cnt]:
                ...
                cnt += 1
        probs_hat = torch.from_numpy(
            numpy.array(probs_hat)
        ).to(enc_out.device)
